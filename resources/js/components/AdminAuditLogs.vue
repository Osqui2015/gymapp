<template>
    <div class="min-h-screen bg-gray-50 dark:bg-[var(--color-obsidian-base)] py-8">
        <Breadcrumbs
            :items="[{ label: 'Inicio', href: '/dashboard' }, { label: 'Auditoría' }]"
            ...
            >
                <div class="flex flex-wrap gap-4">
                    <input
                        ...
                        class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-gray-200"
                        @change="fetchLogs"
                    />
                    <select
                ...
                >
                    ...
                    >
                        <template #default="{ item: log }">
                            <div
                                class="bg-white dark:bg-gray-800 p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
                            >
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex items-start gap-3 flex-1 min-w-0">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            >
                                                Modelo: {{ getModelName(log.model_type) }} #{{
                                                    log.model_id
                                                }}
                                            </p>
                                        </div>
                                    ...
                                    <button
                                        ...
                                        class="px-3 py-1 text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors flex-shrink-0"
                                    >
                                        Ver detalles
                                    </button>
                                </div>
                                ...
                                >
                                    ...
                                    >
                                        <div v-if="log.old_values">
                                            ...
                                            <pre
                                                class="text-xs bg-red-50 dark:bg-red-900/20 p-3 rounded-lg overflow-x-auto"
                                        ...
                                        <div v-if="log.new_values">
                                            ...
                                            <pre
                                                class="text-xs bg-green-50 dark:bg-green-900/20 p-3 rounded-lg overflow-x-auto"
                                                >{{ JSON.stringify(log.new_values, null, 2) }}</pre>
                                        </div>
                                    </div>
                                    <div class="mt-3 text-xs text-gray-400">
                                        IP: {{ log.ip_address || 'N/A' }} | User Agent:
                                        {{ log.user_agent || 'N/A' }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <EmptyState
                        ...
                        :description="
                            Object.values(filtros).some((v) => v)
                                ? 'Probá limpiando los filtros para ver todos los registros.'
                                : 'Cuando alguien haga cambios importantes en el sistema, van a quedar registrados acá.'
                        "
                        variant="compact"
                    />
                </div>
...
const getActionColor = (action) => {
    const colors = {
        ...
        exported: 'bg-cyan-100 text-cyan-600',
        renewed: 'bg-emerald-100 text-emerald-600',
    };
