<template>
    <div class="min-h-screen bg-gray-50 dark:bg-[var(--color-obsidian-base)] py-8">
        <Breadcrumbs
            :items="[{ label: 'Inicio', href: '/dashboard' }, { label: 'Membresías' }]"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border border-green-200 dark:border-green-800"
                >
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border border-yellow-200 dark:border-yellow-800"
                >
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border border-red-200 dark:border-red-800"
                >
            ...
            <!-- Filtros -->
            <div
                class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 mb-6 border border-gray-200 dark:border-gray-700"
            >
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span :class="getDiasClass(m)">
                                            ...
                                            <span v-if="m.estado !== 'cancelado'" class="text-xs"
                                                >({{ getDiasRestantes(m) }} días)</span
                                            >
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                        ...
                        >
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white capitalize"
                                    >{{ m.tipo_plan }} ·
                                    <span class="font-bold">${{ m.precio }}</span></span
                                >
                            </div>
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span :class="getDiasClass(m)" class="text-right">
                                    {{ formatDate(m.fecha_vencimiento) }}
                                    <span v-if="m.estado !== 'cancelado'" class="text-xs block"
                                        >{{ getDiasRestantes(m) }} días restantes</span
                                    >
                                </span>
                            </div>
        ...
        <transition name="fade">
            ...
            >
                <div class="flex items-center justify-center min-h-screen px-4">
                    ...
                    <div
                        ref="modalRef"
                        class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform sm:align-middle sm:max-w-lg w-full"
                        role="dialog"
                        aria-modal="true"
                    >
                        <div
                        ...
                        >
                            <h3 class="text-xl font-bold text-white">
                                {{ editingId ? 'Editar' : 'Nueva' }} Membresía
                            </h3>
                            <button
                                @click="mostrarModalCrear = false"
                        ...
                        <form @submit.prevent="guardarMembresia" class="p-6 space-y-4">
                            <div>
                                ...
                                >
                                    ...
                                    <option
                                        v-for="u in usuariosSinMembresia"
                                        :key="u.id"
                                        :value="u.id"
                                    >
                                        {{ u.name }} ({{ u.email }})
                            ...
                            <div class="flex justify-end gap-3 pt-4">
                                <button
                                    type="button"
                                    @click="mostrarModalCrear = false"
                                    class="px-6 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg font-medium"
                                >
                                    Cancelar
                                </button>
                                <button
    ...
    return (
        {
            ...
            por_vencer: 'bg-yellow-100 text-yellow-600',
...
const fetchMembresias = async (page = 1) => {
    try {
        ...
        pagination.value = {
            current_page: response.data.membresias.current_page,
            last_page: response.data.membresias.last_page,
        };
    } catch (error) {
...
const fetchUsuariosSinMembresia = async () => {
    try {
        const response = await cachedAxiosGet(
            '/api/admin/usuarios-sin-membresia',
            {},
            { ttl: 60_000 }
        );
