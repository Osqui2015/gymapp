                ...
                <div class="lg:col-span-1 space-y-4">
                    ...
                    >
                        ...
                        <div
                            ...
                            class="mt-2 max-h-64 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg"
                        >
                            <button
                        ...
                        <p
                            v-if="busqueda.length >= 2 && resultados.length === 0"
                        ...
                        >
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    <button
                                        type="button"
                                        @click="duplicarDia(index)"
                                        class="px-2.5 py-1 rounded-lg bg-white/20 hover:bg-white/30 text-white text-xs font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                                        title="Duplicar este día con todos sus ejercicios"
                                    >
                                        <svg
                                            class="w-3.5 h-3.5"
                                            fill="none"
                            ...
                            <div class="p-4">
                                ...
                                <div v-if="dia.ejercicios.length > 0" class="overflow-x-auto mb-4">
                                    <table class="w-full text-sm">
                                        <thead>
                                            ...
                                            >
                                                ...
                                                <th
                                                    class="px-4 py-2 text-center font-semibold text-gray-600 dark:text-gray-400"
                                                >
                                                    Superserie
                                                </th>
                                                <th
                                        ...
                                        >
                                            <tr
                                                ...
                                                :class="[
                                                    ...
                                                    dragOverIndex[`${index}-${ejIndex}`]
                                                        ? 'ring-2 ring-indigo-500 ring-inset'
                                                        : '',
                                                    draggingFromIndex === `${index}-${ejIndex}`
                                                        ? 'opacity-40'
                                                        : '',
                                                ]"
                                            >
                                                <td
                                                    class="px-2 py-2 text-center text-gray-400 cursor-grab select-none"
                                                ...
                                                <td
                                                    class="px-4 py-2 font-medium text-gray-800 dark:text-white"
                                                >
                                                    <span
                                                        class="inline-block w-6 text-xs text-gray-400 font-mono"
                                                        >{{ ejIndex + 1 }}.</span
                                                    >
                                                    {{ ej.ejercicio_nombre }}
                                                </td>
                                            ...
                                            >
                                                <td
                                                    colspan="8"
                                                    class="px-2 py-3 text-center text-xs text-indigo-600 dark:text-indigo-400 italic"
                                                >
                                                    <span v-if="dragOverIndex[`${index}-end`]"
                                    ...
                                    <p
                                        class="mt-1 text-xs text-gray-400 dark:text-gray-500 italic px-1"
                                    >
                                        💡 Arrastrá las filas (≡) para reordenar — también podés
                                        mover ejercicios entre días arrastrándolos al final de otra
                                        lista o soltándolos sobre la cabecera del día.
                                    </p>
                                </div>
                                ...
                                <div class="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                    ...
                                    >
                                        Agregar ejercicio a {{ dia.nombre }}:
                                    </p>
                                    ...
                                    <div
                                        v-if="
                                            dia.resultadosBusqueda &&
                                            dia.resultadosBusqueda.length > 0
                                        "
                                        class="mt-2 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg"
                                    >
                                        <button
                                        ...
                                        >
                                            ...
                                            <span
                                                ...
                                                >{{ ej.equipamiento }}</span
                                            >
                                        </button>
                                    </div>
                                    <p
                                        v-if="
                                            dia.busquedaEjercicio &&
                                            dia.busquedaEjercicio.length >= 2 &&
                                            (!dia.resultadosBusqueda ||
                                                dia.resultadosBusqueda.length === 0)
                                        "
                                        class="mt-2 text-sm text-gray-500 dark:text-gray-400"
...
const getSuperserieRowClass = (grupo) => {
    ...
    switch (grupo) {
        case 1:
            return 'border-l-4 border-indigo-500 dark:border-indigo-400 bg-indigo-50/10 dark:bg-indigo-950/20';
        case 2:
            return 'border-l-4 border-emerald-500 dark:border-emerald-400 bg-emerald-50/10 dark:bg-emerald-950/20';
        case 3:
            return 'border-l-4 border-pink-500 dark:border-pink-400 bg-pink-50/10 dark:bg-pink-950/20';
        case 4:
            return 'border-l-4 border-amber-500 dark:border-amber-400 bg-amber-50/10 dark:bg-amber-950/20';
        default:
            return 'border-l-4 border-gray-500 dark:border-gray-400 bg-gray-50/10 dark:bg-gray-950/20';
    }
};
...
const duplicarDia = (diaIndex) => {
    const origen = diasConfigurados.value[diaIndex];
    if (!origen) return;

    const nuevoIndex = diasConfigurados.value.length + 1;
    const nuevoNombre = `Día ${nuevoIndex}`;
    const ejerciciosClonados = (origen.ejercicios || []).map((e) => ({
        ...e,
    }));
    ...
    rutina.value.modalidad = `${diasConfigurados.value.length} ${diasConfigurados.value.length === 1 ? 'Día' : 'Días'}`;
    toast.success(`"${origen.nombre}" duplicado como "${nuevoNombre}"`);
};
...
const onDrop = (diaIndex, targetEjIndex, event) => {
    ...
    if (
        fromDia === diaIndex &&
        (targetEjIndex === null ? fromEj === targetEjercicios.length - 1 : fromEj === targetEjIndex)
    ) {
    ...
    // Nivel 6: si el drop fue sobre un día con ejercicios que ya tienen
    // `id` (modo edición), persistir el nuevo orden en el backend.
    // Si todavía no tienen id (modo creación), el orden se persiste al
    // guardar la rutina.
    persistirOrdenSiEdicion(diaIndex);
};

const persistirOrdenSiEdicion = async (diaIndex) => {
    const dia = diasConfigurados.value[diaIndex];
    if (!dia) return;

    const itemsConId = dia.ejercicios
        .map((ej, idx) => ({ id: ej.id, orden: idx }))
        .filter((it) => it.id != null);

    if (itemsConId.length < 2) return; // no hay nada que reordenar
    if (!rutina.value.modalidad && !rutina.value.nombre) return; // sin contexto

    // Usamos 'Personalizada' como nivel por defecto (igual que guardarRutina).
    // Si la rutina no tiene nivel asignado, no persistimos: quedaría fuera
    // de scope y no se podría reordenar por API.
    const nivel = 'Personalizada';
    const modalidad = rutina.value.nombre;

    try {
        await axios.post('/api/rutinas/reorder', {
            nivel,
            modalidad,
            dia: dia.nombre,
            items: itemsConId,
        });
        // Actualizamos el `orden` local para que coincida con el backend
        // y evitar re-envíos innecesarios.
        dia.ejercicios.forEach((ej, idx) => {
            ej.orden = idx;
        });
    } catch (err) {
        console.warn('[CrearRutina] No se pudo persistir el orden:', err);
        toast.warning('No se pudo guardar el reordenamiento. Reintentá al guardar la rutina.');
    }
};
