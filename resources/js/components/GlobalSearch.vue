...
<template>
    <div class="relative" ref="rootRef">
        ...
        <div class="relative">
            ...
            <kbd
                v-if="!query"
                class="hidden sm:inline-block absolute right-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 text-xs font-mono bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded border border-gray-300 dark:border-gray-600"
                >⌘ K</kbd
            >
            <button
                v-if="query"
        ...
        <transition name="search-dropdown">
            ...
            >
                ...
                >
                    <svg
                        class="animate-spin w-5 h-5 mx-auto mb-2 text-indigo-600"
                        fill="none"
                        viewBox="0 0 24 24"
                    >
                        <circle
                ...
                <div v-else-if="!query && recientes.length" class="py-2">
                    <div
                        class="px-4 py-2 flex items-center justify-between text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400"
                    >
                        <span>🕐 Recientes</span>
                        <button
                ...
                <!-- Sin resultados -->
                <div
                    v-else-if="query.length >= 2 && !totalResultados"
                    class="p-6 text-center text-sm text-gray-500 dark:text-gray-400"
                >
                                ...
                                <a
                                    ...
                                    :class="{
                                        'bg-indigo-50 dark:bg-indigo-950/30':
                                            indexSeleccionado === indexGlobal(i),
                                    }"
                                >
                                    <span
                                        class="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                                        >🏋️</span
                                    >
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="font-medium text-gray-900 dark:text-white truncate"
                                            v-html="highlight(ej.nombre)"
                                        ></p>
                                        <p
                                            class="text-xs text-gray-500 dark:text-gray-400 truncate"
                                        >
                                            <span v-if="ej.grupo_muscular">{{
                                                ej.grupo_muscular
                                            }}</span>
                                            <span v-if="ej.grupo_muscular && ej.equipamiento">
                                                ·
                                            </span>
                                            <span v-if="ej.equipamiento">{{
                                                ej.equipamiento
                                            }}</span>
                                        </p>
                                    </div>
                        ...
                        <ul>
                            <li v-for="(r, i) in resultados.rutinas" :key="`r-${r.id}`">
                                <a
                                    ...
                                    :class="{
                                        'bg-indigo-50 dark:bg-indigo-950/30':
                                            indexSeleccionado === indexGlobal(i, 'rutinas'),
                                    }"
                                >
                                    <span
                                        class="w-8 h-8 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                                        >📋</span
                                    >
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="font-medium text-gray-900 dark:text-white truncate"
                                            v-html="highlight(`${r.nivel} ${r.modalidad}`)"
                                        ></p>
                        ...
                        <ul>
                            <li v-for="(a, i) in resultados.alumnos" :key="`a-${a.id}`">
                                <a
                                    ...
                                    :class="{
                                        'bg-indigo-50 dark:bg-indigo-950/30':
                                            indexSeleccionado === indexGlobal(i, 'alumnos'),
                                    }"
                                >
                                    <div
                                        class="w-8 h-8 bg-purple-100 dark:bg-purple-900/30 rounded-full flex items-center justify-center flex-shrink-0 text-purple-700 dark:text-purple-300 font-bold text-sm"
                                    >
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="font-medium text-gray-900 dark:text-white truncate"
                                            v-html="highlight(a.name)"
                                        ></p>
                                        <p
                                            class="text-xs text-gray-500 dark:text-gray-400 truncate"
                                        >
                                            @<span v-html="highlight(a.nick)"></span> · {{ a.role }}
                                        </p>
                                    </div>
                                </a>
                            </li>
                ...
                >
                    ...
                    <span v-if="totalResultados" class="font-mono"
                        >{{ totalResultados }} resultado{{ totalResultados === 1 ? '' : 's' }}</span
                    >
                </div>
            </div>
        </transition>
...
const totalResultados = computed(() => {
    return (
        (resultados.value.ejercicios?.length || 0) +
...
const buscar = async () => {
    ...
    try {
        ...
        resultados.value = {
            ...
            rutinas: sortByRelevance(
                data.rutinas || [],
                query.value,
                (r) => `${r.nivel} ${r.modalidad}`
            ),
...
const moverSeleccion = (delta) => {
    ...
    indexSeleccionado.value =
        (indexSeleccionado.value + delta + totalItems.value) % totalItems.value;
};
