...
const fetchData = async () => {
    ...
    try {
        const params = {
            ejercicio_nombre: props.ejercicioNombre,
...
const chart = computed(() => {
    ...
    const path = tl
        .map(
            (p, i) => `${i === 0 ? 'M' : 'L'} ${xs(i).toFixed(1)} ${ys(p.estimated_1rm).toFixed(1)}`
        )
        .join(' ');
    ...
    return {
        path,
        points: tl.map((p, i) => ({ x: xs(i), y: ys(p.estimated_1rm), p })),
        yTicks,
        xTicks,
        w,
        h,
...
<template>
    ...
    >
        <header class="mb-3 flex items-start justify-between gap-3">
            <div class="min-w-0">
                ...
                <p class="text-[11px] text-gray-500 dark:text-gray-400">
                    Fórmula {{ formula === 'lander' ? 'Lander' : 'Epley' }} · últimos
                    {{ months }} meses
                </p>
            </div>
            <div v-if="data && data.total_sets > 0" class="shrink-0 text-right">
                ...
                <p v-if="data.pr_count > 0" class="text-[10px] text-amber-600 dark:text-amber-400">
                    +{{ data.pr_count }} PR{{ data.pr_count > 1 ? 's' : '' }}
                </p>
            </div>
        </header>
        ...
        <div
            v-else-if="error"
            class="rounded-md bg-red-50 p-3 text-sm text-red-600 dark:bg-red-950/30"
        >
            {{ error }}
        </div>
        <div
            v-else-if="!data || data.total_sets === 0"
            class="rounded-md bg-gray-50 p-6 text-center text-sm text-gray-500 dark:bg-gray-900/50"
        >
        ...
        <div v-else>
            ...
            >
                ...
                <g v-for="(pt, i) in chart.points" :key="`p-${i}`">
                    ...
                    <title>
                        {{ pt.p.fecha }} · {{ pt.p.weight }}kg × {{ pt.p.reps }} →
                        {{ pt.p.estimated_1rm }}kg
                    </title>
                </g>
            </svg>
            <p
            ...
            >
                Último:
                <span class="font-semibold text-gray-700 dark:text-gray-200">{{ lastLabel }}</span>
                <span v-if="data.estimated_1rm" class="text-gray-400">
                    · {{ data.estimated_1rm.weight }}kg × {{ data.estimated_1rm.reps }}</span
                >
            </p>
        </div>
    </div>
