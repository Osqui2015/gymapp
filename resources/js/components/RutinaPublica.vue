        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Rutina compartida públicamente
                </p>
            </div>
            ...
            >
                <svg
                    class="animate-spin w-10 h-10 mx-auto text-indigo-600 mb-3"
                    fill="none"
                    viewBox="0 0 24 24"
                >
                    <circle
            ...
            <div
                v-else-if="error"
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 text-center border border-red-200 dark:border-red-800"
            >
                ...
                <a
                    href="/"
                    class="inline-block px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium"
                >
                    Volver al inicio
                </a>
            </div>
            ...
            <div v-else-if="rutina">
                <div
                    class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-2xl p-8 mb-6 text-center text-white"
                >
                    <div class="text-5xl mb-3">📋</div>
                    <h1 class="text-3xl md:text-4xl font-bold mb-2">
                        {{ rutina.nivel }} {{ rutina.modalidad }}
                    </h1>
                    <p v-if="rutina.creador" class="text-indigo-100">
                        Compartida por <strong>{{ rutina.creador.name }}</strong> (@{{
                            rutina.creador.nick
                        }})
                    </p>
                </div>
                ...
                <div
                    v-for="(ejercicios, dia) in rutina.dias"
                    :key="dia"
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-4 border border-gray-200 dark:border-gray-700"
                >
                    <h2
                    ...
                    >
                        ...
                        {{ dia }}
                        <span
                            class="ml-auto text-xs font-medium text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full"
                        >
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                <tr v-for="ej in ejercicios" :key="ej.id">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span
                                            class="px-2 py-1 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded font-semibold"
                                            >{{ ej.series }}</span
                                        >
                                    </td>
                                    <td
                                    ...
                                    <td
                                        class="px-4 py-3 text-center text-orange-600 dark:text-orange-400 font-medium"
                                    >
                                        {{ ej.descanso_min }} min
                                    </td>
                                </tr>
                ...
                <div
                    class="bg-indigo-50 dark:bg-indigo-950/30 rounded-2xl p-6 text-center border border-indigo-200 dark:border-indigo-800"
                >
                    <p class="text-sm text-indigo-700 dark:text-indigo-300 mb-3">
                        ¿Querés esta rutina en tu cuenta?
                    </p>
                    <a
                        v-if="authed"
                        href="/rutinas"
...
onMounted(async () => {
    ...
    authed.value =
        ...
        document.cookie.includes('gymapp_session');
    if (!token) {
