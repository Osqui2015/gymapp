...
<template>
    <div
        class="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 rounded-2xl border border-indigo-200 dark:border-indigo-800/50 p-5"
    >
        <div class="flex items-start gap-3 mb-4">
            <div class="text-3xl">🤖</div>
            <div class="flex-1">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                    Rutinas sugeridas para vos
                </h3>
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Basado en tu historial{{
                        perfil?.dias_por_mes
                            ? ` (${perfil.dias_por_mes} días entrenados este mes)`
                            : ''
                    }}.
                </p>
            </div>
                ...
                <div class="flex items-start justify-between gap-3 mb-2">
                    <div>
                        <div class="flex items-center gap-2 flex-wrap">
                            <span
                                class="text-xs font-bold uppercase text-indigo-600 dark:text-indigo-400"
                            >
                                #{{ idx + 1 }}
                            </span>
        ...
        <div
            v-if="perfil && !loading"
            class="mt-4 pt-4 border-t border-indigo-200/50 dark:border-indigo-800/30 text-xs text-gray-500 dark:text-gray-400"
        >
            <details>
                <summary
                ...
                <div class="mt-2 space-y-1 pl-3">
                    <p>
                        • Nivel estimado:
                        <span class="font-semibold">{{ perfil.nivel_estimado }}</span>
                    </p>
                    <p>
                        • Días por mes: <span class="font-semibold">{{ perfil.dias_por_mes }}</span>
                    </p>
                    <p v-if="Object.keys(perfil.top_grupos || {}).length">
                        • Grupos principales:
                        <span
                            v-for="(pct, grupo) in perfil.top_grupos"
                            :key="grupo"
                            class="font-semibold"
                        >
