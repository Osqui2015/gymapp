<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <Breadcrumbs
            :items="[{ label: 'Inicio', href: '/dashboard' }, { label: 'Mis Alumnos' }]"
            class="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto"
        />
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white">
                    Gestión de Trainer-Alumnos
                </h2>
                <a
                    href="/dashboard"
            ...
            <div v-else>
                ...
                >
                    ...
                    >
                        ...
                        <option v-for="trainer in trainers" :key="trainer.id" :value="trainer.id">
                            {{ trainer.name }} ({{
                                trainer.role === 'administrador' ? 'Admin' : 'Trainer'
                            }})
                        </option>
                    </select>
                </div>
                ...
                <div
                    v-if="userRole === 'administrador' && trainerSeleccionado"
                ...
                >
                    ...
                    <div
                        class="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4"
                    >
                        <input
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                ...
                                >
                                    ...
                                    <th
                                        class="px-4 py-4 text-left font-bold text-gray-700 dark:text-gray-300"
                                    >
                                        Alumno
                                    </th>
                                    <th
                                        class="px-4 py-4 text-center font-bold text-gray-700 dark:text-gray-300"
                                    >
                                        Email
                                    </th>
                                    <th
                                ...
                                >
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="bg-indigo-100 dark:bg-indigo-900/30 w-10 h-10 rounded-full flex items-center justify-center"
                                            >
                                                <span
                                                    class="text-indigo-600 dark:text-indigo-400 font-bold"
                                                    >{{ alumno.name.charAt(0).toUpperCase() }}</span
                                                >
                                            </div>
                                            <div>
                                                <p
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    <div
                                        class="bg-indigo-100 dark:bg-indigo-900/30 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                    >
                                        <span
                ...
                <div v-if="userRole !== 'administrador'">
                    <TrainerAlumnosStats
                        ...
                        :alumnos-con-rutina="alumnosConRutina"
                        class="mb-8"
                    />
                    ...
                    >
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                    ...
                                    >
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <span
                                                v-if="alumno.rutina_seleccionada"
                                                class="px-3 py-1 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 rounded-full text-xs font-semibold"
                                            >
                                                {{ alumno.rutina_seleccionada.nivel }}
                                                {{ alumno.rutina_seleccionada.modalidad }}
                                            </span>
                                            <span
                                                v-else
                                                class="px-3 py-1 bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400 rounded-full text-xs font-semibold"
                        ...
                        <div class="md:hidden divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            >
                                <div class="flex items-center gap-3">
                                    <div
                                        class="bg-indigo-100 dark:bg-indigo-900/30 w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0"
                                    >
                                        <span
                                            class="text-indigo-600 dark:text-indigo-400 font-bold"
                                            >{{ alumno.name.charAt(0).toUpperCase() }}</span
                                ...
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span class="text-gray-700 dark:text-gray-300 truncate ml-2">{{
                                        alumno.email
                                    }}</span>
                                </div>
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span
                                        v-if="alumno.rutina_seleccionada"
                                        class="px-2.5 py-1 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 rounded-full text-xs font-semibold"
                                    >
                                        {{ alumno.rutina_seleccionada.nivel }}
                                        {{ alumno.rutina_seleccionada.modalidad }}
                                    </span>
                                    <span
                                        v-else
                                        class="px-2.5 py-1 bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400 rounded-full text-xs font-semibold"
                                    >
                                        Sin asignar
                                    </span>
                                </div>
                        ...
                        <div v-if="alumnosFiltrados.length === 0">
                            <EmptyState
                                ...
                                :description="
                                    busquedaAlumnos
                                        ? 'Probá con otro nombre, nick o email.'
                                        : 'Cuando un admin te asigne alumnos van a aparecer acá para que puedas gestionar sus rutinas.'
                                "
                            />
                        </div>
                    </div>
        ...
        <div
            v-if="mostrarModal && userRole !== 'administrador'"
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                >
                                    ...
                                    <optgroup
                                        v-for="grupo in rutinasAgrupadas"
                                        :key="grupo.etiqueta"
                                        :label="grupo.etiqueta"
                                    >
                                        <option
                                            v-for="rutina in grupo.rutinas"
                                            :key="rutina.id"
                                            :value="rutina.id"
                                        >
                                            {{ rutina.dia }} -
                                ...
                                >
                                    ⚠️ No tienes rutinas creadas. Crea rutinas primero para
                                    asignarlas.
                                </p>
                            </div>

                            <div
                                v-if="rutinaSeleccionada"
                                class="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800"
                            >
                                <h4 class="font-semibold text-indigo-700 dark:text-indigo-300 mb-2">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500"
                                >
                                    <option v-for="n in maxDias" :key="n" :value="`Día ${n}`">
                                        Día {{ n }}
                                    </option>
                                </select>
                            </div>
...
const onRutinaChange = () => {
    if (rutinaForm.value.rutina_id) {
        rutinaSeleccionada.value = rutinas.value.find(
            (r) => r.id === parseInt(rutinaForm.value.rutina_id)
        );
...
const abrirModalAsignar = (alumno) => {
    ...
    if (alumno.rutina_seleccionada && alumno.rutina_seleccionada.rutina_id) {
        ...
        const rutinaExistente = rutinas.value.find(
            (r) => r.id === parseInt(alumno.rutina_seleccionada.rutina_id)
        );
        if (rutinaExistente) {
