    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div v-else>
                ...
                <!-- Lista de Alumnos -->
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700 mb-8"
                >
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        >
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <!-- Filter pills: Estado y Alerta -->
                                <div
                                    class="flex flex-wrap items-center gap-1 bg-gray-100 dark:bg-gray-700/60 p-1 rounded-xl text-xs"
                                >
                                    <button
                                        type="button"
                                    ...
                                    >
                                        Todos ({{ metricas.alumnos?.length || 0 }})
                                    </button>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        Activos ({{ metricas.alumnos_activos || 0 }})
                                    </button>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        Inactivos ({{ metricas.alumnos_inactivos || 0 }})
                                    </button>
                                    <button
                                        type="button"
                                        @click="filtroEstado = 'alerta'"
                                        :class="[
                                            'px-2.5 py-1 rounded-lg font-medium transition-colors',
                                            filtroEstado === 'alerta'
                            ...
                            <input
                                ...
                                class="w-full sm:w-64 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 text-sm"
                            />
                        </div>
                    </div>
                    ...
                    >
                        <template #rows="{ rows }">
                            ...
                            >
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span
                                        v-if="alumno.rutina"
                                        class="px-3 py-1 bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-300 rounded-full text-xs font-semibold"
                                    >
                                        {{ alumno.rutina }}
                                    </span>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span class="text-gray-700 dark:text-gray-300">{{
                                        alumno.dia_actual || 'Día 1'
                                    }}</span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <div class="inline-flex items-center justify-center gap-2">
                                        <div
                                            class="w-14 bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden"
                                        >
                                            <div
                                                class="h-full rounded-full transition-all"
                                                :class="[
                                                    alumno.adherencia_pct >= 70
                                                        ? 'bg-emerald-500'
                                                        : alumno.adherencia_pct >= 40
                                                          ? 'bg-amber-500'
                                                          : 'bg-rose-500',
                                                ]"
                                                :style="{ width: `${alumno.adherencia_pct || 0}%` }"
                                            ></div>
                                        </div>
                                        <span
                                            class="text-xs font-bold font-mono text-gray-700 dark:text-gray-300"
                                        >
                                            {{ alumno.adherencia_pct ?? 0 }}%
                                        </span>
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <div class="inline-flex items-center justify-center gap-1.5">
                                        ...
                                        <span
                                            v-if="alumno.tiene_alerta"
                                            class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300"
                                            :title="`${alumno.dias_sin_entrenar === 999 ? 'Nunca entrenó' : alumno.dias_sin_entrenar + ' días sin entrenar'}`"
                                        >
                                            ⚠️
                                            {{
                                                alumno.dias_sin_entrenar === 999
                                                    ? 'Sin registros'
                                                    : alumno.dias_sin_entrenar + 'd'
                                            }}
                                        </span>
                                    </div>
                                </td>
                        ...
                        <template #cards="{ rows }">
                            <div
                                v-for="alumno in rows"
                                :key="alumno.id"
                                class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 space-y-3"
                            >
                                ...
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span
                                        v-if="alumno.rutina"
                                        class="px-2.5 py-1 bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-300 rounded-full text-xs font-semibold"
                                    >
                                        {{ alumno.rutina }}
                                    </span>
                                    <span v-else class="text-gray-400 dark:text-gray-500"
                        ...
                        <EmptyState
                            ...
                            :description="
                                busqueda
                                    ...
                                    : 'Cuando un admin te asigne alumnos van a aparecer acá.'
                            "
                            variant="compact"
                        />
                    </div>
        ...
        <transition name="fade">
            ...
            >
                <div
                    class="flex items-start justify-center min-h-screen px-4 pt-8 pb-20 text-center sm:block sm:p-0"
                >
                    <div
                    ...
                    <div
                        ref="modalRef"
                        class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-5xl sm:w-full"
                        role="dialog"
                        aria-modal="true"
                    >
                        <!-- Header del Modal -->
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                >
                                    <span class="text-white text-xl font-bold">{{
                                        alumnoSeleccionado.name?.charAt(0).toUpperCase()
                                    }}</span>
                                </div>
                                <div class="text-white">
                                    <h3 class="text-xl font-bold">{{ alumnoSeleccionado.name }}</h3>
                                    <p class="text-indigo-200 text-sm">
                                        @{{ alumnoSeleccionado.nick }}
                                    </p>
                                </div>
                            </div>
                            <button
                        ...
                        <div class="p-6 max-h-[70vh] overflow-y-auto">
                            ...
                            <div v-else>
                                <!-- Tabs de Navegación -->
                                <div
                                    class="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-700 pb-2"
                                >
                                    <button
                                        v-for="tab in tabs"
                                        :key="tab.id"
                                ...
                                <div v-if="tabActivo === 'pesos'">
                                    ...
                                    <div
                                        v-if="
                                            Object.keys(detalleAlumno.historial_pesos || {})
                                                .length > 0
                                        "
                                        class="space-y-4"
                                    >
                                        <div
                                            v-for="(
                                                registros, ejercicio
                                            ) in detalleAlumno.historial_pesos"
                                            :key="ejercicio"
                                        ...
                                        >
                                            ...
                                            <div class="space-y-1">
                                                <div
                                                    v-for="reg in registros.slice(-5)"
                                                    :key="reg.fecha"
                                                    class="flex justify-between text-sm"
                                                >
                                                    <span class="text-gray-500 dark:text-gray-400"
                                                        >{{ reg.fecha }} - {{ reg.dia }}</span
                                                    >
                                                    <span
                                ...
                                <div v-if="tabActivo === 'tonelaje'">
                                    ...
                                    <div
                                        v-if="detalleAlumno.tonelaje_sesiones?.length > 0"
                                        class="space-y-2"
                                    >
                                        <div
                                ...
                                <div v-if="tabActivo === 'medidas'">
                                    ...
                                    <div v-if="detalleAlumno.medidas_corporales?.length > 0">
                                        <ResponsiveTable
                                            :rows="
                                                detalleAlumno.medidas_corporales
                                                    .slice(-10)
                                                    .reverse()
                                            "
                                            :columns="[
                                        ...
                                        >
                                            <template #rows="{ rows }">
                                                <tr
                                                    v-for="medida in rows"
                                                    :key="medida.id ?? medida.fecha"
                                            ...
                                            <template #cards="{ rows }">
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        class="pt-2 border-t border-gray-100 dark:border-gray-700 grid grid-cols-5 gap-2 text-center"
                                                    >
                                                        <div>
                                                            <p
                                                                class="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase"
                                                            >
                                                                Peso
                                                            </p>
                                                            <p
                                                        ...
                                                        <div>
                                                            ...
                                                            <p
                                                                class="text-sm font-medium text-gray-700 dark:text-gray-300 mt-0.5"
                                                            >
                                                                {{ medida.pecho || '-' }}
                                                            </p>
                                                        </div>
                                    ...
                                    <p v-else class="text-gray-500 dark:text-gray-400">
                                        No hay registro de medidas aún.
                                    </p>
                                </div>
                                ...
                                <div v-if="tabActivo === 'historial'">
                                    ...
                                    <div
                                        v-if="detalleAlumno.historial_completo?.length > 0"
                                        class="space-y-3"
                                    >
                                        <div
                                            v-for="item in detalleAlumno.historial_completo"
                                            :key="item.id"
                                            class="border border-gray-200 dark:border-gray-700 rounded-lg p-4"
                                        >
                                            ...
                                            >
                                                <div class="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span
                                                        class="font-semibold text-yellow-800 dark:text-yellow-300 text-sm"
                                                        >{{
                                                            item.trainer_nombre || 'Trainer'
                                                        }}</span
                                                    >
                                                </div>
                                                <p
...
const busqueda = ref('');
const filtroEstado = ref('todos'); // 'todos' | 'activos' | 'inactivos' | 'alerta'

const totalAlertas = computed(() => {
    return (metricas.value.alumnos || []).filter((a) => a.tiene_alerta).length;
});
...
const alumnosFiltrados = computed(() => {
    let list = metricas.value.alumnos || [];

    if (filtroEstado.value === 'activos') {
        list = list.filter((a) => a.activo_semana);
    } else if (filtroEstado.value === 'inactivos') {
        list = list.filter((a) => !a.activo_semana);
    } else if (filtroEstado.value === 'alerta') {
        list = list.filter((a) => a.tiene_alerta);
    }
