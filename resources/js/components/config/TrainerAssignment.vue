<template>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <!-- Trainers list -->
        <div
            class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden lg:col-span-1"
        >
            ...
            <div
                class="p-4 space-y-2 max-h-[550px] overflow-y-auto bg-gray-50/50 dark:bg-gray-900/10"
            >
                <div
                ...
                >
                    <div>
                        ...
                        <p class="text-xs text-gray-400">
                            @{{ t.nick }} • <span class="capitalize">{{ t.role }}</span>
                        </p>
                    </div>
                    <svg
                        v-if="selectedTrainer?.id === t.id"
        ...
        <!-- Alumnos -->
        <div
            class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden lg:col-span-2"
        >
            <div
                class="px-6 py-4 bg-gradient-to-r from-purple-500 to-indigo-500 flex justify-between items-center"
            >
                <h2 class="text-xl font-bold text-white flex items-center gap-2">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
            ...
            <div v-if="!selectedTrainer" class="p-12 text-center text-gray-500 dark:text-gray-400">
                ...
                <p class="text-sm text-gray-400 mt-1">
                    Para gestionar, activar o modificar su listado de alumnos.
                </p>
            </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                            Alumnos de: {{ selectedTrainer.name }}
                        </h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            Marca los alumnos que entrenarán bajo el cargo de este entrenador.
                        </p>
                    </div>
                    <button
                        @click="$emit('guardar-asignacion')"
                        :disabled="guardando"
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            ...
                            class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-xs font-semibold transition-colors"
                        >
                            Seleccionar Todos
                        </button>
                        <button
                            @click="$emit('desmarcar-todos')"
                            class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-xs font-semibold transition-colors"
                        >
                            Desmarcar Todos
                        </button>
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex-1 flex items-center justify-between gap-4">
                            ...
                            <div
                                v-if="a.trainer_id && a.trainer_id !== selectedTrainer.id"
                                class="text-right"
                            >
                                <span
                                    class="inline-flex items-center rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-semibold text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-900/40"
                                    >Asignado a: {{ a.trainer?.name }}</span
                                >
                            </div>
                            <div v-else-if="a.trainer_id === selectedTrainer.id" class="text-right">
                                <span
                                    class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-800 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-900/40"
                                    >Activo</span
                                >
                            </div>
                            <div v-else class="text-right">
                                <span
                                    class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-500 dark:bg-gray-800 dark:text-gray-400"
                                    >Sin Asignar</span
                                >
                            </div>
                        </div>
