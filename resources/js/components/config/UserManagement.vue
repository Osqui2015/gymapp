    ...
    <div>
        ...
        >
            ...
            >
                ...
                <form
                    @submit.prevent="$emit('crear', nuevoUsuario)"
                    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fadeIn"
                >
                    <CampoForm
                        v-model="nuevoUsuario.nick"
                        label="Nick *"
                        type="text"
                        required
                    ...
                    <div v-if="nuevoUsuario.role === 'alumno'">
                        <label
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                            >Trainer Asignado</label
                        >
                        <select
                            v-model="nuevoUsuario.trainer_id"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        >
                    ...
                    <div class="md:col-span-2 lg:col-span-3 flex items-end pt-2">
                        <button
                            type="submit"
                            :disabled="creando"
                            class="ripple px-6 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-semibold rounded-lg transition-colors flex items-center gap-2"
                        >
                            <svg
                                v-if="creando"
                ...
                <div
                    v-if="errorCrear"
                    class="mt-4 p-4 bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-lg text-red-700 dark:text-red-300"
                >
                    {{ errorCrear }}
                </div>
                <div
                    v-if="successCrear"
                    class="mt-4 p-4 bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-700 rounded-lg text-green-700 dark:text-green-300"
                >
                    {{ successCrear }}
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div
                    class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 hidden md:block"
                >
                            ...
                            >
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold text-sm"
                                        >
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span
                                        class="px-2 py-1 text-xs font-bold rounded-full capitalize"
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-center">
                                    <span
                                        v-if="u.suspended"
                                        class="px-2 py-1 text-xs font-bold rounded-full bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300"
                                        >Suspendido</span
                                    >
                                    <span
                                        v-else
                                        class="px-2 py-1 text-xs font-bold rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300"
                                        >Activo</span
                                    >
                                </td>
                                <td
                                ...
                                >
                                    <div class="inline-flex gap-1">
                                        <button
                                            @click="$emit('abrir-editar', u)"
                                            class="p-1.5 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-lg transition-colors"
                                        ...
                                        <button
                                            ...
                                            class="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors"
                ...
                <!-- Mobile: cards -->
                <div
                    class="md:hidden rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700 overflow-hidden"
                >
                    <div
                        v-for="u in users"
                        :key="u.id"
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold flex-shrink-0"
                            >
                                {{ (u.name || u.nick || '?').charAt(0).toUpperCase() }}
                            </div>
                            ...
                            <span
                                class="px-2 py-1 text-xs font-bold rounded-full capitalize flex-shrink-0"
                                :class="roleClass(u.role)"
                                >{{ u.role }}</span
                            >
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="inline-flex gap-1">
                                <button
                                    @click="$emit('abrir-editar', u)"
                                    class="p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-lg transition-colors"
                                    :aria-label="`Editar usuario ${u.name}`"
                                    title="Editar"
                                >
                                    <svg
                        ...
                        <EmptyState
                            ...
                            :description="
                                searchFilter
                                    ...
                                    : 'Cuando se registren usuarios van a aparecer acá para que puedas gestionarlos.'
                            "
                            variant="compact"
                        />
                    </div>
        ...
        <Teleport to="body">
            ...
            >
                <div
                    class="fixed inset-0 bg-gray-900/75 dark:bg-gray-950/80 transition-opacity"
                ></div>

                <div
                    ref="modalRef"
                    class="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-2xl transform transition-all sm:max-w-lg w-full relative z-10 border border-gray-200 dark:border-gray-700"
                    role="dialog"
                    aria-modal="true"
                >
                    <div
                        class="px-6 py-4 bg-gradient-to-r from-indigo-500 to-purple-500 text-white flex justify-between items-center"
                    >
                        <h3 class="text-lg font-bold">
                            Editar Usuario: @{{ formularioEdit.nick }}
                        </h3>
                        <button
                    ...
                    <form
                        @submit.prevent="$emit('guardar-edicion', formularioEdit)"
                        class="p-6 space-y-4"
                    >
                        <CampoForm
                        ...
                        />
                        <CampoForm
                            v-model="formularioEdit.telefono"
                            label="Teléfono"
                            type="text"
                            placeholder="11-1234-5678"
                        />
