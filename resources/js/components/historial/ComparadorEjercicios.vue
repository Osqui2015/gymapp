<template>
    ...
    >
        <div class="flex items-center gap-2 mb-3">
            ...
            <div>
                <h3 class="text-base font-bold text-gray-800 dark:text-white">
                    Comparar progreso por ejercicio
                </h3>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    Cuánto subiste entre dos fechas en un ejercicio específico
                </p>
            </div>
        </div>
        ...
        <div v-else-if="resultado">
            <div
                v-if="resultado.desde_stats.sets === 0 && resultado.hasta_stats.sets === 0"
                class="text-center text-sm text-gray-500 dark:text-gray-400 py-6"
            >
                No hay sets registrados para <strong>{{ resultado.ejercicio }}</strong> en el
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="rounded-xl bg-gray-50 dark:bg-[var(--color-obsidian-base)]/40 p-4 text-center">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        <span class="text-xs text-gray-500"
                            >{{ formatNum(resultado.desde_stats.peso_max) }}kg</span
                        >
                        <svg
                        ...
                        <span class="text-xs text-gray-500"
                            >{{ formatNum(resultado.hasta_stats.peso_max) }}kg</span
                        >
                    </div>
                    <p
                        :class="[
                            'text-3xl font-black font-mono mt-1',
                            diffClass(resultado.diff.peso_max),
                        ]"
                    >
                ...
                <div class="rounded-xl bg-gray-50 dark:bg-[var(--color-obsidian-base)]/40 p-4 text-center">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        <span class="text-xs text-gray-500">{{
                            formatNum(resultado.desde_stats.volumen_total)
                        }}</span>
                        <svg
                            class="w-4 h-4 text-gray-400"
                            fill="none"
                        ...
                        <span class="text-xs text-gray-500">{{
                            formatNum(resultado.hasta_stats.volumen_total)
                        }}</span>
                    </div>
                    <p
                        :class="[
                            'text-3xl font-black font-mono mt-1',
                            diffClass(resultado.diff.volumen_total),
                        ]"
                    >
                ...
                <div class="rounded-xl bg-gray-50 dark:bg-[var(--color-obsidian-base)]/40 p-4 text-center">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        <span class="text-xs text-gray-500">{{
                            formatNum(resultado.desde_stats.reps_promedio)
                        }}</span>
                        <svg
                            class="w-4 h-4 text-gray-400"
                            fill="none"
                        ...
                        <span class="text-xs text-gray-500">{{
                            formatNum(resultado.hasta_stats.reps_promedio)
                        }}</span>
                    </div>
                    <p
                        :class="[
                            'text-3xl font-black font-mono mt-1',
                            diffClass(resultado.diff.reps_promedio, true),
                        ]"
                    >
            ...
            <p class="text-[10px] text-gray-400 text-center mt-3">
                ...
                días, {{ resultado.desde_stats.sets + resultado.hasta_stats.sets }} sets totales).
            </p>
        </div>
...
const ejerciciosDisponibles = computed(() => {
    const set = new Set();
    (props.ejercicios || []).forEach((e) =>
        set.add(typeof e === 'string' ? e : e.ejercicio_nombre || e.nombre)
    );
...
const diffClass = (delta, invertirColor = false) => {
    ...
    return positivo ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400';
};
