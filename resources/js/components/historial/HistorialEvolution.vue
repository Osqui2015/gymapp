<template>
    <div class="space-y-6">
        ...
        >
            <div
                class="px-6 py-4 bg-gradient-to-r from-slate-900 to-indigo-900 border-b border-gray-200 dark:border-gray-700"
            >
            ...
            <div class="p-6">
                <ResponsiveTable
                    ...
                    :columns="[
                        ...
                        {
                            key: 'maxWeight',
                            label: 'Peso Máximo Cargado',
                            thClass: 'text-right text-indigo-600 dark:text-indigo-400 font-bold',
                            tdClass: '',
                            sortable: true,
                        },
                    ]"
                ...
                >
                    <template #rows="{ rows }">
                        ...
                        >
                            ...
                            <td
                                class="px-4 py-3 text-right font-black text-indigo-600 dark:text-indigo-400 text-base"
                            >
                                {{ fila.maxWeight }} kg
                            </td>
                        </tr>
                        ...
                        >
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    ...
                                    <span
                                        v-if="fila.superserie_grupo"
                                        class="mt-1 inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 ring-1 ring-inset ring-indigo-700/10 dark:bg-indigo-900/40 dark:text-indigo-400"
                                    >
                                        Superserie {{ fila.superserie_grupo }}
                                    </span>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    ...
                                    <p
                                        class="font-black text-indigo-600 dark:text-indigo-400 text-lg leading-tight"
                                    >
                                        {{ fila.maxWeight }} kg
                                    </p>
                                </div>
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {{ ejercicio.totalSeries }} series registradas ·
                            {{ ejercicio.totalSesiones }} sesiones
                        </p>
                    </div>

                    <div class="flex flex-wrap gap-2 text-xs md:text-sm">
                        <span
                            class="rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200 px-2.5 py-1 font-medium"
                        >
                            Peso promedio: {{ ejercicio.pesoPromedio }} kg
                        </span>
                        <span
                            class="rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-200 px-2.5 py-1 font-medium"
                        >
                            Reps promedio: {{ ejercicio.repsPromedio }}
                        </span>
                        <span
                            class="rounded-full bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200 px-2.5 py-1 font-medium"
                        >
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                            Evolución del peso promedio por fecha
                        </p>
                    ...
                    <div class="overflow-x-auto">
                        ...
                        >
                            <defs>
                                <linearGradient
                                    :id="`bar-gradient-${ejercicio.slug}`"
                                    x1="0"
                                    y1="0"
                                    x2="0"
                                    y2="1"
                            ...
                            <g
                                ...
                                :key="`bar-${ejercicio.slug}-${index}`"
                            >
                                <rect
                                    :x="
                                        barX(index, ejercicio.timeline.length) -
                                        barWidth(ejercicio.timeline.length) / 2
                                    "
                            ...
                            <g v-if="ejercicio.timeline.length">
                                <g
                                    v-for="(point, index) in ejercicio.timeline"
                                    :key="`label-${ejercicio.slug}-${index}`"
                                >
                                    <text
                                    ...
                                    <text
                                        :x="barX(index, ejercicio.timeline.length)"
                                        :y="scaleBarY(point.avgPeso, globalMaxWeight) - 10"
                                        text-anchor="middle"
                                        fill="#e2e8f0"
                                        font-size="11"
                                        font-weight="700"
                            ...
                            >
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <div class="text-right text-sm">
                                        ...
                                        >
                                            {{ sesion.pesoPromedio }} kg
                                        </p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ sesion.repsPromedio }} reps promedio
                                        </p>
                                    </div>
                                </div>
                            </div>
