<template>
    <div>
        <!-- Hero / header card -->
        <div
            class="rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 border border-slate-800 shadow-2xl p-6 md:p-8 mb-8"
        >
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    <p class="text-sm uppercase tracking-[0.2em] text-indigo-300 mb-2">
                    ...
                    <p class="mt-3 text-sm md:text-base text-slate-300 max-w-2xl">
                        Revisa la evolución de tus ejercicios, el promedio de peso utilizado y cómo
                        cambia el rendimiento por día.
                    </p>

                    <!-- Selector de alumno para entrenadores/admins -->
                    <div
                        v-if="isTrainerOrAdmin && alumnos.length"
                        class="mt-6 flex flex-col sm:flex-row sm:items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-4 max-w-xl"
                    >
        ...
        <!-- Stats grid -->
        <div class="grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 mb-8">
            <div
                class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm"
            >
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Ejercicios</p>
            ...
            <div
                class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm"
            >
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Series</p>
                <p
                    class="mt-2 text-2xl sm:text-3xl font-bold text-emerald-600 dark:text-emerald-400"
                >
                    {{ stats.totalSeries }}
                </p>
            </div>
            ...
            >
                ...
                <p class="mt-2 text-2xl sm:text-3xl font-bold text-purple-600 dark:text-purple-400">
                    {{ stats.tonelajeTotal || '0 kg' }}
                </p>
            </div>
            <div
                class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm"
            >
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Peso prom.</p>
                <p class="mt-2 text-2xl sm:text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                    {{ stats.pesoPromedio }} kg
                </p>
            </div>
            <div
                class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm col-span-2 sm:col-span-1"
            >
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Reps prom.</p>
                <p class="mt-2 text-2xl sm:text-3xl font-bold text-orange-600 dark:text-orange-400">
                    {{ stats.repsPromedio }}
                </p>
            </div>
        </div>
        ...
        <!-- Tabs -->
        <div
            class="flex border-b border-gray-200 dark:border-gray-700 mb-6 gap-6 overflow-x-auto scrollbar-hide"
        >
            <button
                v-for="tab in tabs"
                :key="tab.id"
            ...
            >
                ...
                >
                    <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M12 4a2 2 0 100 4 2 2 0 000-4zM8 12c0-2 2-3 4-3s4 1 4 3v6a1 1 0 11-2 0v-2H10v2a1 1 0 11-2 0v-6z"
                    />
                </svg>
                <svg
                    v-else-if="tab.icon === 'key'"
                    class="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
