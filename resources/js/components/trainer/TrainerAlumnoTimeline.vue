<template>
    <div class="space-y-4">
        ...
        <div
            v-else
            class="relative pl-6 border-l-2 border-indigo-200 dark:border-indigo-800 space-y-4"
        >
            <div v-for="(ev, i) in eventosPaginados" :key="i" class="relative">
                <!-- Dot -->
                <div
                    :class="[
                        'absolute -left-[33px] top-1 w-4 h-4 rounded-full ring-4 ring-white dark:ring-gray-900 flex items-center justify-center text-xs',
                        ev.color,
                    ]"
                >
                ...
                >
                    ...
                    <p v-if="ev.descripcion" class="mt-1 text-xs text-gray-600 dark:text-gray-400">
                        {{ ev.descripcion }}
                    </p>
                    <div v-if="ev.meta" class="mt-2 flex flex-wrap gap-2 text-xs">
                        <span
                            v-for="(val, k) in ev.meta"
                            :key="k"
                            class="px-2 py-0.5 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700"
                        >
...
const eventosPaginados = computed(() =>
    mostrarTodos.value ? eventos.value : eventos.value.slice(0, pageSize)
);
